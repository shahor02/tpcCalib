...
HistoManager* HistoManager::CreateClone(const char* prefix)
{
  ...
  hm->AddPrefix(prefix);
  TH1* histo;
  for (int i=0;i<GetLast()+1;i++) if ((histo=GetHisto(i))) histo->SetDirectory(0);
...
void HistoManager::Write(TFile* file)
{
  ...
  for (int i=0;i<GetLast()+1;i++) {
    if (!(histo=GetHisto(i))) continue;
    TDirectory* dr = histo->GetDirectory();
    histo->SetDirectory(0);
    histo->Write(0,TObject::kOverwrite);
    if (dr) histo->SetDirectory(dr);
  }
