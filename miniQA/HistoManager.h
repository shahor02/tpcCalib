...
: public TObjArray
{
 ...
 public:
  ...
  TGraph* GetGraph(Int_t id)       const {return id<=GetLast() ? (TGraph*) UncheckedAt(id) : 0;}
  TH1*    GetHisto(Int_t id)       const {return id<=GetLast() ? (TH1*) UncheckedAt(id) : 0;}
  TH1*    GetHisto(char* name)     const {return (TH1*) FindObject(name);}
  TH1F*   GetHisto1F(Int_t id)     const {return (TH1F*) GetHisto(id);}
  TH2F*   GetHisto2F(Int_t id)     const {return (TH2F*) GetHisto(id);}
  TProfile* GetHistoP(Int_t id)    const {return (TProfile*) GetHisto(id);}
