...
void Process(int ev)
{
  ...
  if (!trRejRS) {
    printf("Q2PT: %+.3f %.3f %.4f %d\n",q2pt,q2ptFit,arrPhi[0],arrSectID[0]);
  ...
  delete grTrZ;
  delete grTrY;
  grTrY = new TGraph(nCl,arrX,arrYtrack);
  //  grTrZ = new TGraph(nCl,arrXtrack,arrZtrack);
  grTrZ = new TGraph(nCl,arrX,arrZtrack);
		      ...
		      float *resHelixY, float *resHelixZ, float maxDevY, float maxDevZ)
{
  ...
  // work in the frame of 1st sector the track is located
  ...
  float hmnY=1e9,hmxY=-1e9,hmnZ=1e9,hmxZ=-1e9;
  float sna=0,csa=0; // sector alpha sin and cos
  float phiSect = 0;
  for (int ip=0;ip<np;ip++) {
  ...
  //
  printf("q/pt: %+6.3f np:%3d Y: %+.4f %+.4f | Z: %+.4f %+.4f\n",q2ptFit,np,hmnY,hmxY,hmnZ,hmxZ);
	       ...
	       float &xc, float &yc, float &r2, float* dy)
{
  ...
  //
  // determine qurvature
  float phi0 = TMath::ATan2(y[0]-yc,x[0]-xc);
  if (phi0<0) phi0 += TMath::Pi()*2;
  float phi1 = TMath::ATan2(y[np-1]-yc,x[np-1]-xc);
  if (phi1<0) phi1 += TMath::Pi()*2;
  int curvSign = 1;
  if (phi1>phi0 || phi0-phi1>TMath::Pi()) curvSign = -1;
